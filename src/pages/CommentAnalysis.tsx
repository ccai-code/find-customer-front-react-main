...
const CommentAnalysis: React.FC<CommentAnalysisProps> = ({ taskId }) => {
  ...
  const {
    ...
    pageType: 'analysis',
    // 只在组件初始化时设置一次，避免后续变化导致的覆盖
    initialSelectedTaskId: null, // 移除动态值，避免循环更新
    onTaskSelect: useCallback((taskId: string) => {
      // 保存选中的任务ID到localStorage，供私信页面使用
      localStorage.setItem('analysis_selectedTaskId', taskId);
      // 同时同步到私信页面的localStorage，确保一致性
      ...
      if (response.status === 200 && response.data?.comment_list) {
        const mappedComments = response.data.comment_list.map((comment: any) => {
          ...
          return {
            ...
            '评论时间': getFieldValue('评论时间', ['comment_time', 'time']),
            '用户昵称': getFieldValue('用户昵称', ['user_name', 'nickname']),
            'IP地址': getFieldValue('IP地址', ['user_region', 'province']),
            '评论内容': getFieldValue('评论内容', ['comment_content', 'content']),
            '用户链接': getFieldValue('用户链接', ['user_link']),
            '内容链接': getFieldValue('内容链接', ['content_link']),
            'intent_customer': comment.intent_customer || comment.intent || '待分析',
            '分析理由': getFieldValue('分析理由', ['analysis_reason', 'reason'])
          };
        });
  ...
  // 监听分析进度变化，当达到100%时刷新任务列表状态和评论列表
  useEffect(() => {
  ...
  const fetchAnalysisProgress = useCallback(async (taskId: string) => {
    if (!taskId || !autoRefreshEnabled) return;
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        // 直接根据 num/sum 计算进度百分比
        const progress = sum > 0 ? Math.round((num / sum) * 100) : 0;
  ...
  }, [autoRefreshEnabled, lastRefreshTime, refreshTimerId]); // 添加依赖
  ...
  useEffect(() => {
    ...
    // 更新下次刷新时间
    const updateNextRefreshTime = (interval: number) => {
        ...
        if (isCompleted) {
          ...
          const newIntervalId = setInterval(async () => {
            ...
            try {
              ...
              updateNextRefreshTime(NORMAL_INTERVAL);
            } catch (error) {
              retryCount++;
              if (retryCount >= MAX_RETRIES) {
                ...
                updateNextRefreshTime(RETRY_DELAY);
              }
            }
          }, NORMAL_INTERVAL);
          
          setRefreshTimerId(newIntervalId);
          updateNextRefreshTime(NORMAL_INTERVAL);
        }
      } catch (error) {
