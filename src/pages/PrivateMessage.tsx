...
import { postLoginDouyin, getIsAllStatus, updateIsAllStatus, openDouyinWebsite } from '../api/privateMessageApi'; // 导入登录抖音账号API和is_all状态管理API
  ...
  const columns = [
    { 
      ...
      render: (text: string, record: DetailItem) => {
        // 如果有用户ID，使用openDouyinWebsite打开
        if (record.user_id) {
          return (
            <a 
              onClick={() => openDouyinWebsite(record.user_id)}
              style={{ 
                color: '#1890ff', 
                textDecoration: 'none',
                cursor: 'pointer',
